...
interface SortControlsProps<TData> {
  table: Table<TData>
  debounceMs: number
  shallow?: boolean
}

export default function SortControls<TData>({
  table,
  debounceMs,
  shallow,
}: SortControlsProps<TData>) {
  const uniqueSortId = React.useId()
  const uniqueLabelId = React.useId()
  const uniqueDescriptionId = React.useId()
    ...
    >
      <Popover>
        ...
        <PopoverContent
          ...
          align='start'
          collisionPadding={16}
          className='flex w-[calc(100vw-(--spacing(20)))] max-w-[25rem] min-w-72 origin-(--radix-popover-content-transform-origin) flex-col gap-3.5 p-4 sm:w-[25rem]'
        >
          <div className='flex flex-col gap-1'>
            <h4 id={uniqueLabelId} className='leading-none font-medium'>
            ...
            <SortableContent asChild>
              ...
              >
                {uniqueSorting.map((sort) => {
                  const sortItemId = `${uniqueSortId}-sort-${sort.id}`
                        ...
                        <Popover>
                          <PopoverTrigger asChild>
                            <Button
                              ...
                              variant='outline'
                              size='sm'
                              className='focus:ring-ring h-8 w-44 justify-between rounded focus:ring-1 focus:outline-hidden'
                              onPointerDown={(event) => {
                                ...
                                <CommandGroup>
                                  {sortableColumns.map((column) => (
                                    <CommandItem
                                      ...
                                      value={column.id}
                                      onSelect={(value) => {
                                        const newFieldTriggerId = `${uniqueSortId}-sort-${value}-field-trigger`
